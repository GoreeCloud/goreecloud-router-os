...
| P1 | GL.iNet Brume 3 (GL-MT5000) ARM64 hardware qualification | Development | Development hardware profile, fail-closed readiness validator, non-mutating stock-firmware discovery collector, privacy/status validator, direct-observation summarizer, source-reported recovery/board evidence, and fixture safety tests implemented on the active candidate branch | Repository CI and direct physical-hardware evidence required; vendor-documented recovery and a closed-unmerged vendor OpenWrt proposal do not establish recovery/kernel/hardware acceptance; no bootable image, install profile, or Supported status accepted |
